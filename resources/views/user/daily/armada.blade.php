...
<div class="page-inner">
    <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row pt-2 pb-4">
        <div>
            <h3 class="fw-bold mb-3">Laporan Harian Armada Item</h3>
            <h6 class="op-7 mb-2">Tanggal: {{ \Carbon\Carbon::parse($date)->format('d M Y') }}</h6>
        </div>
        <div class="ms-md-auto py-2 py-md-0">
            <form method="GET" action="{{ route('user.daily.armada') }}" class="d-flex gap-2">
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title mb-0">Armada Item</h4>
                </div>
                <div class="card-body">
                    @if(session('success'))
                    ...
                    @forelse($armadaItems as $index => $item)
                        @if($index === 0)
                            <form id="bulk-armada-report" action="{{ route('user.daily.store-multiple') }}" method="POST" enctype="multipart/form-data" class="bulk-report-form">
                                @csrf
                                <input type="hidden" name="subject_type" value="armada" />
                                <input type="hidden" name="date" value="{{ $date }}" />
                                <input type="hidden" name="scroll_y" value="0" class="scroll-pos-input" />
                        @endif

                        @php $key = App\Models\ArmadaItem::class . '_' . $item->id; $report = $reports[$key] ?? null; @endphp
                        
                        <div class="border rounded p-3 mb-3 armada-item-section" data-item-id="{{ $item->id }}">
                            <div class="mb-3">
                                {{-- menambahkan angka sesuai dengan urutan bukan dengan ID --}}
                                <h5 class="mb-2"><span class="badge bg-primary me-2">{{ $loop->iteration }}</span>Armada Item</h5>
                                @if($item->car)
                                    <div class="text-muted mb-2">
                                        <i class="fas fa-shipping-fast"></i> Mobil: <strong>{{ $item->car->nama_mobil ?? '-' }}</strong> / {{ $item->car->plat_nomor ?? '-' }}
                                    </div>
                                @endif
                        ...
                        @endif
                    @empty
                        <p class="text-muted">Tidak ada armada item.</p>
                    @endforelse
                </div>
            </div>
        </div>
...
// apply styles based on status
        ...
        document.querySelectorAll('.capture-photo-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                // atur ukuran camvas sesuai video 
                ...
                canvas.toBlob(function(blob) {
                    ...
                    // Buat DataTransfer untuk mengatur input file
        ...
        // Hentikan semua kamera saat meninggalkan halaman
