...
<div class="page-inner">
    <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row pt-2 pb-4">
        <div>
            <h3 class="fw-bold mb-3">Laporan Harian Dokumen</h3>
            <h6 class="op-7 mb-2">Tanggal: {{ \Carbon\Carbon::parse($date)->format('d M Y') }}</h6>
        </div>
        <div class="ms-md-auto py-2 py-md-0">
            <form method="GET" action="{{ route('user.daily.dokument') }}" class="d-flex gap-2">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    @forelse($dokuments as $index => $item)
                        @if($index === 0)
                            <form id="bulk-dokument-report" action="{{ route('user.daily.store-multiple') }}" method="POST" enctype="multipart/form-data" class="bulk-report-form">
                                @csrf
                        ...
                        @php $key = App\Models\Dokument::class . '_' . $item->id; $report = $reports[$key] ?? null; @endphp
                        
                        <div class="border rounded p-3 mb-3 dokument-item-section" data-item-id="{{ $item->id }}">
                            <div class="mb-3">
                                <h5 class="mb-2"><span class="badge bg-primary me-2">{{ $loop->iteration }}</span>Document Item</h5>
                        ...
                        @endif
                    @empty
                        <p class="text-muted">Tidak ada dokument.</p>
                    @endforelse
                </div>
            </div>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        let activeStreams = {}; // Menyimpan data/video dari kamera yang lagi nyala.
        function applyStatusStyles(selectEl) {
            ...
            // Memperbarui status validasi pada select (dropdown).
        ...
        // Inisialisasi dan pasang pendengar untuk semua select status
            ...
            btn.addEventListener('click', async function() {
                ...
                try {
                    ...
                    const stream = await navigator.mediaDevices.getUserMedia({ 
                        video: { facingMode: 'environment' }, // Pakai kamera belakang di HP
                        audio: false 
                    });
        ...
        document.querySelectorAll('.capture-photo-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                // Atur canvas supaya pas sama ukuran video
                ...
                // ubah canvas jadi blob (file/data)
                canvas.toBlob(function(blob) {
                    // Bikin file dari blob
                    const file = new File([blob], `camera-photo-${Date.now()}.jpg`, { type: 'image/jpeg' });
                    
                    // Bikin DataTransfer buat ngisi input file.
        ...
        // Tangani perubahan input file untuk menampilkan preview
        ...
        // Matikan semua kamera saat keluar dari halaman
