...
<div class="page-inner">
    <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row pt-2 pb-4">
        <div>
            <h3 class="fw-bold mb-3">Laporan Harian Safety</h3>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    @forelse($safetys as $index => $item)
                        @if($index === 0)
                            <form id="bulk-safety-report" action="{{ route('user.daily.store-multiple') }}" method="POST" enctype="multipart/form-data" class="bulk-report-form">
                                @csrf
                        ...
                        @php $key = App\Models\Safety::class . '_' . $item->id; $report = $reports[$key] ?? null; @endphp
                        
                        <div class="border rounded p-3 mb-3 safety-item-section" data-item-id="{{ $item->id }}">
                            <div class="mb-3">
                                <h5 class="mb-2"><span class="badge bg-primary me-2">{{ $loop->iteration }}</span>Safety Item</h5>
                        ...
                        @endif
                    @empty
                        <p class="text-muted">Tidak ada safety.</p>
                    @endforelse
                </div>
            </div>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // Biarkan browser mengatur scroll ke bagian tertentu secara otomatis
        history.scrollRestoration = 'manual';

        let activeStreams = {}; // menyimpan stream kamera aktif perindex
        ...
        function applyStatusStyles(selectEl) {
            ...
            // Perbarui status validasi select
